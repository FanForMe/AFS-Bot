...
Client.on("message", message => {
	...
	if( msg.toLowerCase().startsWith( prefix + '=roleremove' ) ){
		...
		if( !role1 ) return message.reply( '**:x: يرجى وضع الرتبة المراد سحبها من الشخص**' );if( message.mentions.members.first() ){
			...
			return message.reply('**:white_check_mark: [ '+role.owner+' ] owner [ '+args[0]+' ] تم سحب من **');
		}
		if( args[0].toLowerCase() == "=removeall" ){
			message.guild.members.forEach(m=>m.removeRole( role1 ))
			return	message.reply('**:white_check_mark: [ '+role1.owner+' ] تم سحب من الكل رتبة**');
		} else if( args[0].toLowerCase() == "bots" ){
			message.guild.members.filter(m=>m.user.bot).forEach(m=>m.removeRole(role1))
			return	message.reply('**:white_check_mark: [ '+role1.owner+' ] تم سحب من البوتات رتبة**');
		} else if( args[0].toLowerCase() == "humans" ){
			message.guild.members.filter(m=>!m.user.bot).forEach(m=>m.removeRole(role1))
			return	message.reply('**:white_check_mark: [ '+role1.owner+' ] تم سحب من البشريين رتبة**');
		} 	
	} else {
		...
		if( args[0].toLowerCase() == "=addall" ){
			...
			return	message.reply('**:white_check_mark: [ '+role1.owner+' ] تم اعطاء الكل رتبة**');
		} else if( args[0].toLowerCase() == "bots" ){
			message.guild.members.filter(m=>m.user.bot).forEach(m=>m.addRole(role1))
			return	message.reply('**:white_check_mark: [ '+role1.owner+' ] تم اعطاء البوتات رتبة**');
		} else if( args[0].toLowerCase() == "humans" ){
			message.guild.members.filter(m=>!m.user.bot).forEach(m=>m.addRole(role1))
			return	message.reply('**:white_check_mark: [ '+role1.owner+' ] تم إعطاء الشخص رتبة**');
		} 
	} 
});
